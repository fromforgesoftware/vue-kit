<template>
	<CalendarRoot
		v-model:placeholder="placeholder"
		:model-value="rekaValue"
		:min-value="rekaMin"
		:max-value="rekaMax"
		:locale="locale"
		:number-of-months="numberOfMonths"
		:week-starts-on="resolvedWeekStart"
		:fixed-weeks="fixedWeeks"
		:disabled="disabled"
		:is-date-disabled="rekaIsDateDisabled"
		:is-date-unavailable="rekaIsDateUnavailable"
		data-slot="calendar"
		:class="rootClasses"
		@update:model-value="onUpdate"
	>
		<template #default="{ grid, weekDays, date }">
			<CalendarHeader data-slot="calendar-header" :class="calendarHeaderVariants()">
				<CalendarPrev
					data-slot="calendar-prev"
					aria-label="Previous month"
					:class="navButtonClasses"
				>
					<Icon :icon="ChevronLeft" size="sm" />
				</CalendarPrev>

				<template v-if="monthYearSelect">
					<div data-slot="calendar-month-year-select" class="flex items-center gap-1">
						<select
							:value="date.month"
							data-slot="calendar-month-select"
							aria-label="Month"
							:class="selectClasses"
							@change="onMonthChange($event, date)"
						>
							<option v-for="m in monthNames" :key="m.value" :value="m.value">
								{{ m.label }}
							</option>
						</select>
						<select
							:value="date.year"
							data-slot="calendar-year-select"
							aria-label="Year"
							:class="selectClasses"
							@change="onYearChange($event, date)"
						>
							<option v-for="y in yearRange" :key="y" :value="y">
								{{ y }}
							</option>
						</select>
					</div>
				</template>
				<CalendarHeading v-else data-slot="calendar-heading" :class="headingClasses" />

				<CalendarNext data-slot="calendar-next" aria-label="Next month" :class="navButtonClasses">
					<Icon :icon="ChevronRight" size="sm" />
				</CalendarNext>
			</CalendarHeader>

			<div class="flex flex-col space-y-4 pt-4 sm:flex-row sm:space-x-4 sm:space-y-0">
				<CalendarGrid
					v-for="month in grid"
					:key="month.value.toString()"
					data-slot="calendar-grid"
					:class="calendarGridVariants()"
				>
					<CalendarGridHead data-slot="calendar-grid-head">
						<CalendarGridRow :class="cn(calendarGridRowVariants(), 'mb-1')">
							<CalendarHeadCell
								v-for="day in weekDays"
								:key="day"
								data-slot="calendar-head-cell"
								:class="headCellClasses"
							>
								{{ day }}
							</CalendarHeadCell>
						</CalendarGridRow>
					</CalendarGridHead>
					<CalendarGridBody data-slot="calendar-grid-body" class="grid">
						<CalendarGridRow
							v-for="(week, weekIdx) in month.rows"
							:key="`week-${weekIdx}`"
							data-slot="calendar-grid-row"
							:class="calendarGridRowVariants()"
						>
							<CalendarCell
								v-for="day in week"
								:key="day.toString()"
								:date="day"
								data-slot="calendar-cell"
								:class="cellClasses"
							>
								<CalendarCellTrigger
									:day="day"
									:month="month.value"
									data-slot="calendar-cell-trigger"
									:class="cellTriggerClasses"
								/>
							</CalendarCell>
						</CalendarGridRow>
					</CalendarGridBody>
				</CalendarGrid>
			</div>
		</template>
	</CalendarRoot>
</template>

<script setup lang="ts">
import { computed, ref } from 'vue';
import { ChevronLeft, ChevronRight } from '@lucide/vue';
import Icon from '../../general/icon/Icon.vue';
import {
	CalendarRoot,
	CalendarHeader,
	CalendarHeading,
	CalendarPrev,
	CalendarNext,
	CalendarGrid,
	CalendarGridHead,
	CalendarGridBody,
	CalendarGridRow,
	CalendarHeadCell,
	CalendarCell,
	CalendarCellTrigger,
} from 'reka-ui';
import { CalendarDate, type DateValue } from '@internationalized/date';
import { type ForgeDate, getWeekStartForCalendar } from '@fromforgesoftware/ts-kit';
import { cn } from '../../../helpers/cn.js';
import { toRekaDate, calendarDateToForgeDate } from '../_adapter.js';
import {
	calendarRootVariants,
	calendarHeaderVariants,
	calendarHeadingVariants,
	calendarSelectVariants,
	calendarNavButtonVariants,
	calendarGridVariants,
	calendarGridRowVariants,
	calendarHeadCellVariants,
	calendarCellVariants,
	calendarCellTriggerVariants,
	type CalendarVariants,
} from './calendar.js';

interface Props {
	/** Selected date (or null when nothing is selected). */
	modelValue?: ForgeDate | null;
	/** Earliest selectable date. Days before this are visually disabled. */
	minValue?: ForgeDate;
	/** Latest selectable date. Days after this are visually disabled. */
	maxValue?: ForgeDate;
	/** BCP-47 locale used for month/weekday names. */
	locale?: string;
	/** Number of months to render side-by-side (1 or 2). */
	numberOfMonths?: number;
	/** First day of week (0 = Sunday … 6 = Saturday). Defaults to user preference. */
	weekStartsOn?: 0 | 1 | 2 | 3 | 4 | 5 | 6;
	/** Pads weeks so every month has 6 rows — keeps height stable. */
	fixedWeeks?: boolean;
	/** Disable all interaction. */
	disabled?: boolean;
	/** Predicate marking a date as disabled (returned class is muted, not interactive). */
	isDateDisabled?: (date: ForgeDate) => boolean;
	/** Predicate marking a date as unavailable (struck-through, not interactive). */
	isDateUnavailable?: (date: ForgeDate) => boolean;
	/** Replace the heading label with month + year `<select>` controls. */
	monthYearSelect?: boolean;
	/** Density. `sm` = 28px cells, `default` = 32px, `lg` = 36px. */
	size?: CalendarVariants['size'];
	/** Extra wrapper classes (layout only — propose a variant for visual changes). */
	class?: string;
}

const props = withDefaults(defineProps<Props>(), {
	locale: 'en',
	numberOfMonths: 1,
	fixedWeeks: true,
	disabled: false,
	monthYearSelect: false,
	size: 'default',
});

// Falls back to the user's locale week-start; the calendar grid layout depends on this value.
const resolvedWeekStart = computed(() => props.weekStartsOn ?? getWeekStartForCalendar());

const emit = defineEmits<{
	'update:modelValue': [value: ForgeDate];
}>();

const placeholder = ref<DateValue>();

const rekaValue = computed(() => toRekaDate(props.modelValue));
const rekaMin = computed(() => toRekaDate(props.minValue));
const rekaMax = computed(() => toRekaDate(props.maxValue));

const rekaIsDateDisabled = computed(() => {
	if (!props.isDateDisabled) return undefined;
	return (date: DateValue) => props.isDateDisabled!(calendarDateToForgeDate(date));
});

const rekaIsDateUnavailable = computed(() => {
	if (!props.isDateUnavailable) return undefined;
	return (date: DateValue) => props.isDateUnavailable!(calendarDateToForgeDate(date));
});

function onUpdate(value: DateValue | undefined) {
	if (!value) return;
	emit('update:modelValue', calendarDateToForgeDate(value));
}

const monthNames = computed(() => {
	const formatter = new Intl.DateTimeFormat(props.locale, { month: 'long' });
	return Array.from({ length: 12 }, (_, i) => ({
		value: i + 1,
		label: formatter.format(new Date(2000, i, 1)),
	}));
});

const yearRange = computed(() => {
	const current = placeholder.value?.year ?? new Date().getFullYear();
	const start = current - 100;
	const end = current + 10;
	return Array.from({ length: end - start + 1 }, (_, i) => start + i);
});

function onMonthChange(event: Event, date: DateValue) {
	const month = Number((event.target as HTMLSelectElement).value);
	placeholder.value = new CalendarDate(date.year, month, 1);
}

function onYearChange(event: Event, date: DateValue) {
	const year = Number((event.target as HTMLSelectElement).value);
	placeholder.value = new CalendarDate(year, date.month, 1);
}

const rootClasses = computed(() => cn(calendarRootVariants({ size: props.size }), props.class));
const headingClasses = computed(() => calendarHeadingVariants({ size: props.size }));
const navButtonClasses = computed(() => calendarNavButtonVariants({ size: props.size }));
const selectClasses = computed(() => calendarSelectVariants({ size: props.size }));
const headCellClasses = computed(() => calendarHeadCellVariants({ size: props.size }));
const cellClasses = computed(() => calendarCellVariants({ size: props.size }));
const cellTriggerClasses = computed(() => calendarCellTriggerVariants({ size: props.size }));
</script>
