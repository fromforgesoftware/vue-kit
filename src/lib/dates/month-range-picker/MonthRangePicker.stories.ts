...
const meta = {
	...
	parameters: {
		docs: {
			description: {
				component:
					'Composite month-range picker — button trigger that opens a popover with a 4×3 month grid. Built on Reka UI primitives.',
			},
		},
	},
	render: (args) => ({
		...
		template: `
      <div class="space-y-2">
        <MonthRangePicker v-bind="args" v-model="value" @update:modelValue="args['onUpdate:modelValue']" />
...
export const Variants: Story = {
	render: (args) => ({
      ...
      <div class="flex flex-wrap items-start gap-4">
        <MonthRangePicker v-bind="args" variant="button" />
        <MonthRangePicker v-bind="args" variant="inline" />
      </div>
    `,
	}),
};
...
export const Sizes: Story = {
	render: (args) => ({
      ...
      <div class="flex flex-wrap items-start gap-3">
        <MonthRangePicker v-bind="args" size="sm" />
        <MonthRangePicker v-bind="args" size="default" />
        <MonthRangePicker v-bind="args" size="lg" />
      </div>
    `,
	}),
};
...
export const WithError: Story = {
	args: { error: true, describedBy: 'month-range-error' },
	render: (args) => ({
		components: { MonthRangePicker, Label },
		setup() {
			const value = ref<{ start: ForgeDate; end: ForgeDate } | null>(null);
			return { args, value };
		},
      ...
      <div class="grid w-72 gap-2">
        ...
        <MonthRangePicker v-bind="args" v-model="value" />
        <p id="month-range-error" class="text-xs text-destructive">Please pick a range.</p>
      </div>
    `,
	}),
};
...
export const WithMinMax: Story = {
	...
	render: (args) => ({
		...
		setup() {
			const value = ref<{ start: ForgeDate; end: ForgeDate } | null>(null);
			const min = ForgeDate.now().minus({ months: 12 });
			const max = ForgeDate.now().plus({ months: 12 });
			return { args, value, min, max };
		},
		template: `<MonthRangePicker v-bind="args" v-model="value" :min-value="min" :max-value="max" />`,
	}),
};
...
export const InteractiveSelectEmitsModel: Story = {
	...
	play: async ({ args, canvasElement }) => {
		...
		const cells = document.body.querySelectorAll<HTMLElement>(
			'[data-slot="month-range-picker-cell-trigger"]:not([data-disabled])',
		);
...
export const InteractiveOutsideClickCloses: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		const open = document.body.querySelector(
			'[data-slot="month-range-picker-content"][data-state="open"]',
		);
...
export const InteractiveResponsive: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await forEachViewport(async () => {
			...
			const trigger = root.querySelector('[data-slot="month-range-picker-trigger"]') as HTMLElement;
