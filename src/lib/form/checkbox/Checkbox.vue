<template>
	<CheckboxRoot
		:id="id"
		data-slot="checkbox"
		:model-value="modelValue"
		:default-value="defaultChecked"
		:disabled="disabled"
		:required="required"
		:name="name"
		:value="value"
		:aria-invalid="error || undefined"
		:aria-label="ariaLabel"
		:aria-describedby="describedBy"
		:class="classes"
		@update:model-value="onModelValueChange"
	>
		<CheckboxIndicator data-slot="checkbox-indicator" :class="indicatorClasses">
			<svg
				v-if="checked === 'indeterminate'"
				viewBox="0 0 9 9"
				fill="currentColor"
				xmlns="http://www.w3.org/2000/svg"
				aria-hidden="true"
			>
				<path
					fill-rule="evenodd"
					clip-rule="evenodd"
					d="M0.75 4.5C0.75 4.08579 1.08579 3.75 1.5 3.75H7.5C7.91421 3.75 8.25 4.08579 8.25 4.5C8.25 4.91421 7.91421 5.25 7.5 5.25H1.5C1.08579 5.25 0.75 4.91421 0.75 4.5Z"
				/>
			</svg>
			<svg
				v-else
				viewBox="0 0 9 9"
				fill="currentColor"
				xmlns="http://www.w3.org/2000/svg"
				aria-hidden="true"
			>
				<path
					fill-rule="evenodd"
					clip-rule="evenodd"
					d="M8.53547 0.62293C8.88226 0.849446 8.97976 1.3142 8.75325 1.66099L4.5083 8.1599C4.38833 8.34356 4.19397 8.4655 3.9764 8.49358C3.75883 8.52167 3.53987 8.45309 3.3772 8.30591L0.616113 5.80777C0.308959 5.52987 0.285246 5.05559 0.563148 4.74844C0.84105 4.44128 1.31533 4.41757 1.62249 4.69547L3.73256 6.60459L7.49741 0.840706C7.72393 0.493916 8.18868 0.396414 8.53547 0.62293Z"
				/>
			</svg>
		</CheckboxIndicator>
	</CheckboxRoot>
</template>

<script setup lang="ts">
import { computed } from 'vue';
import { CheckboxRoot, CheckboxIndicator } from 'reka-ui';
import { cn } from '../../../helpers/cn';
import { checkboxVariants, checkboxIndicatorVariants, type CheckboxVariants } from './checkbox';

interface CheckboxProps {
	/** Controlled state. Pass `'indeterminate'` for the dash variant. */
	checked?: boolean | 'indeterminate';
	/** Initial uncontrolled state. */
	defaultChecked?: boolean;
	/** Disabled state. */
	disabled?: boolean;
	/** Required attribute (native form validation). */
	required?: boolean;
	/** Sets `aria-invalid="true"` and tints the border destructive. */
	error?: boolean;
	/** id of the element(s) describing this checkbox — error or hint text. */
	describedBy?: string;
	/** Density. `default` = 16 px painted square; `sm` = 14 px. Both sizes meet WCAG 24×24 hit-area via invisible padding. */
	size?: CheckboxVariants['size'];
	/** Native form `name` attribute. */
	name?: string;
	/** Native form `value` attribute. */
	value?: string;
	/** id matching the `for` of an associated `<Label>`. */
	id?: string;
	/**
	 * Accessible name used when there is no visible <Label> (e.g. inside a table
	 * header or icon-only context). Vue does not auto-forward camelCase
	 * `ariaLabel` to component children, so we accept it explicitly.
	 */
	ariaLabel?: string;
	/** Extra classes. */
	class?: string;
}

const props = withDefaults(defineProps<CheckboxProps>(), {
	checked: undefined,
	defaultChecked: false,
	disabled: false,
	required: false,
	error: false,
	size: 'default',
});

const emit = defineEmits<{
	'update:checked': [value: boolean | 'indeterminate'];
}>();

const classes = computed(() => cn(checkboxVariants({ size: props.size }), props.class));
const indicatorClasses = computed(() => cn(checkboxIndicatorVariants({ size: props.size })));

const modelValue = computed<boolean | 'indeterminate'>(() => {
	if (props.checked === 'indeterminate') return 'indeterminate';
	return props.checked === true;
});

function onModelValueChange(val: boolean | 'indeterminate') {
	emit('update:checked', val);
}
</script>
