import type { Meta, StoryObj } from '@storybook/vue3-vite';
import { expect, userEvent } from 'storybook/test';
import AuthRegister from './AuthRegister.vue';
import { expectMinTargetSize } from '../../../test-utils/playHelpers';

const meta = {
	title: 'Authentication/Register',
	component: AuthRegister,
	tags: ['!autodocs'],
	parameters: { layout: 'fullscreen' },
	argTypes: {
		loading: { control: 'boolean' },
	},
} satisfies Meta<typeof AuthRegister>;

export default meta;
type Story = StoryObj<typeof meta>;

export const Register: Story = {
	render: (args) => ({
		components: { AuthRegister },
		setup() {
			return { args };
		},
		template: `
      <div class="flex min-h-screen items-center justify-center bg-muted px-4 py-12">
        <AuthRegister v-bind="args" />
      </div>
    `,
	}),
	args: { loading: false },
};

export const RegisterLoading: Story = {
	render: (args) => ({
		components: { AuthRegister },
		setup() {
			return { args };
		},
		template: `
      <div class="flex min-h-screen items-center justify-center bg-muted px-4 py-12">
        <AuthRegister v-bind="args" />
      </div>
    `,
	}),
	args: { loading: true },
};

export const InteractiveTargetSize: Story = {
	tags: ['!autodocs', 'test'],
	render: (args) => ({
		components: { AuthRegister },
		setup() {
			return { args };
		},
		template: `
      <div class="flex min-h-screen items-center justify-center bg-muted px-4 py-12">
        <AuthRegister v-bind="args" />
      </div>
    `,
	}),
	args: { loading: false },
	play: async ({ canvasElement }) => {
		const submit = canvasElement.querySelector('[data-slot="auth-submit"]') as HTMLElement;
		await expect(submit).toBeInTheDocument();
		expectMinTargetSize(submit, 24);
	},
};

export const InteractiveTabOrder: Story = {
	tags: ['!autodocs', 'test'],
	render: (args) => ({
		components: { AuthRegister },
		setup() {
			return { args };
		},
		template: `
      <div class="flex min-h-screen items-center justify-center bg-muted px-4 py-12">
        <AuthRegister v-bind="args" />
      </div>
    `,
	}),
	args: { loading: false },
	play: async ({ canvasElement }) => {
		const name = canvasElement.querySelector('#register-name') as HTMLInputElement;
		const email = canvasElement.querySelector('#register-email') as HTMLInputElement;
		name.focus();
		await expect(name).toHaveFocus();
		await userEvent.tab();
		await expect(email).toHaveFocus();
	},
};
